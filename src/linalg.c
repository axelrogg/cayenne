...
int linalg_luf(double **matrix, int n, double **matrix_lu) {
    ...
    if (matrix_lu == NULL) {
        printf("linalg.c :: lufactor :: could not allocate memory to create matrix LUI.\n");
        return -1;
    }
    ...
    for (int k = 0; k < n; k++) {
        ...
        // This sets the values of matrix U into `matrix_lu` (with a modification).
        ...
        // we cannot satisfy that requirement (the diagonal of `matrix_lu` at this
        ...
        // This is what makes `matrix_lu` = L + U - I.
        for (int j = k + 1; j < n; j++) {
            double sum = 0;
            for (int t = 0; t < k; t++) {
...
double linalg_det(double **matrix, int n) {
    ...
    double **matrix_lu = malloc(n * sizeof(double*));
    if (matrix_lu == NULL) {
        printf("linalg.c :: linalg_det :: matrix_lu is NULL.\n");
        return NAN;
    }

    if (mat_init_zeroes(matrix_lu, n, n) == -1) {
        mat_free(matrix_lu, n);
...
int linalg_lineq_solve(double **matrix, int n, double *b, double *x) {
    ...
    matrix_lu = malloc(n * sizeof(double *));
    if (matrix_lu == NULL) {
